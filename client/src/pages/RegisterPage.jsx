...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const referrerId = localStorage.getItem('cc_referrer')
      if (referrerId) {
        api.post('/referrals/register', { referrerId }).catch(() => {})
        localStorage.removeItem('cc_referrer')
      }
      navigate('/game')
    } catch (err) {
  ...
  return (
    <div className="auth-page">
      <div className="card auth-card">
        ...
        <div style={{marginTop:8}}>¿Ya tienes cuenta? <Link to="/login">Inicia sesión</Link></div>
        </div>
      </div>
    </div>
  )
